...
if [ "${CREATED}" -ne "0" ] || [ "${DELETED}" -ne "0" ] || [ "${UPDATED}" -gt "2500" ] || [ "$(($SKIPPED / $UPDATED))" -lt "175" ];
then
    # The numbers look odd.
    echo "$(date '+%Y-%m-%d %H:%M:%S') !! The debugging run seems to have had some unexpected results." >> "${LOG}";
    echo "                       Variants were created or deleted, or the high percentage of updated variants is too high." >> "${LOG}";
